...
def get_prof_id(dept,prof):
	cur.execute("SELECT id FROM profs WHERE department_id='"+dept+"' AND display_name='"+MySQLdb.escape_string(prof)+"';")	
	id=cur.fetchone()
	if(id):
		return id[0]
	else:
		cur.execute("INSERT INTO profs (department_id,display_name) VALUES( "+dept+", '"+MySQLdb.escape_string(prof)+"');")
