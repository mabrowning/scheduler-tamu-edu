...
def get_course_id(dept,course,description="")
	...
	if(id):
		...
		cur.execute("INSERT INTO allcourses (number,department_id,description_string) VALUES( "+course+", "+str(dept)+" );")
		cur.execute("SELECT LAST_INSERT_ID();")
		id=cur.fetchone()[0]
...
def insert_section(course,section,dept,semester):
	...
	if isinstance(course,(string)):
		course=get_course_id(dept_id,course)
	if isinstance(section["prof"],(string)):
...
def update_section(course,section,dept):
  ...
  "AND department_id='"+str(dept_id)+"' AND sections.number="+str(section["number"])+"' AND allcourses.number="+str(course)+";")
	id=cur.fetchone()[0]
	section["prof"]=get_prof_id(dept_id,section["prof"])
