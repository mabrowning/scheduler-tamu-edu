...
function Init()
{
  Calender = new CalenderBlock();
  ControllerDIV=document.getElementById('controllerDIV');
  Hourspan=document.getElementById('hours');
}
//This class is the functional calender. It handles sizing and distributing the TimeBlocks.
function CalenderBlock()
{
  this.cal=document.getElementById('cal');
  this.height=this.cal.clientHeight;
  this.start_hour=8;
  this.hours=12;
  //vv pixels per hour
  this.pph=Math.round(this.height/this.hours);
  this.days=[];
  this.days["M"]=document.getElementById('M');
  this.days["T"]=document.getElementById('T');
  this.days["W"]=document.getElementById('W');
  this.days["R"]=document.getElementById('R');
  this.days["F"]=document.getElementById('F');
  
  this.Position = function(start)
  {
    return Math.round((start.Hours()-this.start_hour)*this.pph)+'px';
  } 
  this.Height = function(start,stop)
  {
    return Math.round((stop.Hours()-start.Hours())*this.pph-1)+'px';
  }
}
